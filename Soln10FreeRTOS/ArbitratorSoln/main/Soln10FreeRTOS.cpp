...
static SemaphoreHandle_t arbitrator_mutex = NULL; // arbitrator_mutex
...
void eat(void *parameters) {
  ...
  if (xSemaphoreTake(arbitrator_mutex, portMAX_DELAY)) {
    // Take left chopstick
    xSemaphoreTake(chopstick[left], portMAX_DELAY);
    ESP_LOGI(Philosopher_task_TAG.data(), "Philosopher %u took chopstick %u",
             num, left);
    ...
    ESP_LOGI(Philosopher_task_TAG.data(),
             "Philosopher %u returned chopstick %u", num, right);

    // Put down left chopstick
    xSemaphoreGive(chopstick[left]);
    ESP_LOGI(Philosopher_task_TAG.data(),
             "Philosopher %u returned chopstick %u", num, left);
